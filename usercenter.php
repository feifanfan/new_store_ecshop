...
function action_edit_list(){
	...
	$sql = "select user_id,parent_id,user_name,user_rank,mobile_phone,user_status from ecs_users where user_id =$id";
	$data = $db->getRow($sql);

	$ext_sql = "select * from ecs_bd_log where bd_id = $user_id and user_id =$id";
	$ext = $db->getRow($ext_sql);
	if($ext == ''){
		$smarty->assign('exist', 1);
	}
	$sqls = "select user_upgrade,user_money from ecs_users where user_id =$user_id";
	$res = $db->getRow($sqls);

	$pu = $GLOBALS['db']->getOne("select value from ecs_shop_config where code='a_card_buynum'");
	$yin = $GLOBALS['db']->getOne("select value from ecs_shop_config where code='b_card_buynum'");
	...
	$smarty->assign('jin', $jin);
	$smarty->assign('zuan', $zuan);
	// var_dump($data);die;
...
function action_zijin(){
	...
	$sql = "select COUNT(*) from ecs_user_account where user_id = $user_id";
	$record_count = $db->getOne($sql);
    $page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;
	// 分页函数
	$pager = get_pager('usercenter.php', array( 'act' => $action ), $record_count, $page);

	$sql = "select * from ecs_user_account where user_id = $user_id ORDER BY add_time desc limit $pager[start],$pager[size]";
	$data = $db->getAll($sql);
	...
	{
		//时间戳转为时间格式
		$data[$i]['add_time'] = date($GLOBALS['_CFG']['date_format'], $data[$i]['add_time']);
	}
	//查询消费金额
	$jiner = "select sum(amount) from ecs_user_account where user_id = $user_id and is_paid =1";
	$xiaofei = $db->getOne($jiner);
	// echo'<pre>';var_dump($data);die;
	$smarty->assign('zijinlist', $data);
	$smarty->assign('pager', $pager);
	$smarty->assign('xiaofei', abs($xiaofei));
	$smarty->assign('info', get_user_default($user_id));	
	$smarty->display('user_center.dwt');
}

/* 对会员余额申请提现的处理 */
function action_chuli_tixian(){
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	$smarty = $GLOBALS['smarty'];
	...
	$ecs = $GLOBALS['ecs'];
	$user_id = $_SESSION['user_id'];
	$shouxufei = isset($_POST['shouxufei']) ? trim($_POST['shouxufei']) : 0;
	$amount = isset($_POST['amount']) ? trim($_POST['amount']) : '';
	$user_note = isset($_POST['user_note']) ? trim($_POST['user_note']) : '';
	$add_time = time();
	$sxf = $amount * $shouxufei/100;
	$insert_sql = "insert into ".$GLOBALS['ecs']->table("user_account")." (user_id,amount,add_time,user_note,process_type,is_paid,money) values ( '$user_id','-$amount','$add_time','$user_note','1','0','$sxf')";
	// $up_sql = "update ".$GLOBALS['ecs']->table('users')." set user_money = user_money-".$money." where user_id = ".$user_id;
	$res = $GLOBALS['db']->query($insert_sql);	
	// $GLOBALS['db']->query($up_sql);	
	if($res){
		show_message('申请成功,等待管理员审核', '返回上一页', 'usercenter.php?act=zijin');
	}else{
		show_message('申请失败', '返回上一页', 'usercenter.php?act=zijin');
	}
}

function action_act_bd_check(){
	// 获取全局变量
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	...
	$bd_upgrade = isset($_POST['shengjibi']) ? trim($_POST['shengjibi']) : '';
	$bd_money = isset($_POST['xianjin']) ? trim($_POST['xianjin']) : '';
	$bd_id =$user_id;
	$bd_time =time();

	$sql = "insert into " . $ecs->table('bd_log') . "(user_id, bd_id, bd_money, bd_upgrade, bd_time) " . " values('$uid', '$bd_id', '$bd_money', '$bd_upgrade', '$bd_time')";
	$res = $db->query($sql);
	if($res){
		show_message('提交成功,等待审核', '返回上一页', 'usercenter.php?act=bd_list');
	}else{
		show_message('提交失败', '返回上一页', 'usercenter.php?act=bd_list');
	}

}
//激活
function action_jihuo(){
	// 获取全局变量
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
...
function action_ji_huo(){
	...
	if($res){
		...
		show_message('激活失败', '返回上一页', 'usercenter.php');
	}
}
...
function action_shengji(){
	...
	$sqls = "select user_upgrade,user_rank from ecs_users where user_id =$user_id";
	$level = $db->getRow($sqls);
...
function action_gonggao(){
	...
	$sql = "select COUNT(*) from ecs_article where cat_id = 99";
	$record_count = $db->getOne($sql);
    $page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;
	// 分页函数
	$pager = get_pager('usercenter.php', array( 'act' => $action ), $record_count, $page);

	$sql = "select * from ecs_article where cat_id = 99 limit $pager[start],$pager[size]";
	$data = $db->getAll($sql);
	...
	// echo'<pre>';var_dump($data);die;
	$smarty->assign('gonggaolist', $data);
	$smarty->assign('pager', $pager);
	$smarty->display('user_center.dwt');
}
//查看公告
function action_gonggao_list(){
	// 获取全局变量
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	...
	$data = $db->getRow($sql);
	//时间戳转为时间格式
	$data['add_time'] = date($GLOBALS['_CFG']['date_format'], $data['add_time']);
...
function action_act_exchange_check(){
	// 获取全局变量
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	...
	$qiyebi = isset($_POST['qiyebi']) ? trim($_POST['qiyebi']) : '';

	$zong = $cash + $qiyebi;
	$sql = "update " . $ecs->table('users') . " set user_cash = user_cash-".$cash.",user_point = user_point-".$qiyebi.",pay_points = pay_points+".$zong." where user_id='$user_id' ";
	$res = $db->query($sql);
	$sqls = "select user_name from ecs_users where user_id =$user_id";
	$log = $db->getOne($sqls);
	$change_time = time();
	if($res){
		if($cash !=0 && $qiyebi !=0){
		...
		show_message('转化成功', '返回上一页', 'usercenter.php?act=exchange');
	}else{
		show_message('转化失败', '返回上一页', 'usercenter.php?act=exchange');
	}
}
//升级等级操作
function action_act_level(){
	// 获取全局变量
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	...
	$upgrade = isset($_POST['upgrade']) ? trim($_POST['upgrade']) : 0;
	$curlevel = isset($_POST['curlevel']) ? trim($_POST['curlevel']) : 0;
	if($curlevel == 1 && $level ==2){
		$num = $yin - $pu;
	}elseif($curlevel == 1 && $level ==3){
		$num = $jin - $pu;
	}elseif($curlevel == 1 && $level ==4){
		$num = $zuan - $pu;
	}elseif($curlevel == 2 && $level ==3){
		$num = $jin - $yin;
	}elseif($curlevel == 2 && $level ==4){
		$num = $zuan - $yin;
	}elseif($curlevel == 3 && $level ==4){
		$num = $zuan - $jin;
	}
	$sql = "update " . $ecs->table('users') . " set user_rank = $level,user_point = user_point+".$num.",user_upgrade = user_upgrade-".$num." where user_id='$user_id' ";
	$res = $db->query($sql);
	$sqls = "select user_name from ecs_users where user_id =$user_id";
	$log = $db->getOne($sqls);
	...
	if($res){
		show_message('升级成功', '返回上一页', 'usercenter.php?act=shengji');
	}else{
		show_message('升级失败', '返回上一页', 'usercenter.php?act=shengji');
	}
}
...
function action_jifen_list ()
{
	...
	$sql = "select * from ecs_account_log where user_id=$user_id order by change_time desc limit $pager[start],$pager[size]";
	$data = $db->getAll($sql);
	...
	{
		//时间戳转为时间格式
		$data[$i]['change_time'] = date($GLOBALS['_CFG']['date_format'], $data[$i]['change_time']);
	}
	// echo'<pre>';var_dump($data);die;
...
function action_jifen_huizong ()
{
	...
	$action = $GLOBALS['action'];
	$user_id = $_SESSION['user_id'];
	$begintime = isset($_POST['begintime']) ? trim($_POST['begintime']) : '';
	$begin = isset($_POST['begintime']) ? trim($_POST['begintime']) : '';
	...
	if(!empty($begintime) && !empty($endtime)){
		...
		$sqls = "SELECT
		user_id,FROM_UNIXTIME(change_time,'%Y-%m-%d') ctime,sum(user_cash) user_cash,sum(user_money) user_money,
		SUM(CASE WHEN change_type = 94 THEN (user_money + user_cash) ELSE 0 END) baodanfei,
		SUM(CASE WHEN change_type = 95 THEN (user_money + user_cash) ELSE 0 END) cengjiang,
		SUM(CASE WHEN change_type = 96 THEN (user_money + user_cash) ELSE 0 END) duipeng,
		SUM(CASE WHEN change_type = 97 THEN (user_money + user_cash) ELSE 0 END) guanli,
		SUM(CASE WHEN change_type = 98 THEN (user_money + user_cash) ELSE 0 END) jiandian
		FROM ecs_account_log where change_time >= {$endtime} and change_time <= {$begintime} and user_id = $user_id GROUP BY user_id,ctime order by ctime desc limit $pager[start],$pager[size]";
		...
		$sql = "SELECT
				...
				SUM(CASE WHEN change_type = 98 THEN (user_money + user_cash) ELSE 0 END) jiandian
				FROM
				ecs_account_log where user_id = $user_id GROUP BY user_id,ctime order by ctime desc limit $pager[start],$pager[size]";
		$data = $db->getAll($sql);
	}
...
function action_account_raply ()
{
	...
	$user_id = $_SESSION['user_id'];
	include_once (ROOT_PATH . 'includes/lib_clips.php');
	// echo '<pre>';var_dump(get_user_default($user_id));die;
	$shouxufei = $GLOBALS['db']->getOne("select value from ecs_shop_config where code='charge'");
	$smarty->assign('shouxufei', $shouxufei);
	$smarty->assign('info', get_user_default($user_id));
	$smarty->display('user_center.dwt');
}
...
function action_account_detail ()
{
	...
	// 分页函数
	$pager = get_pager('usercenter.php', array(
		'act' => $action
	), $record_count, $page);
	...
	// echo '<pre>';var_dump($account_log);die;
	//查询消费金额
	$jiner = "select sum(amount) from ecs_user_account where user_id = $user_id and is_paid =1";
	$xiaofei = $db->getOne($jiner);
	$smarty->assign('xiaofei', abs($xiaofei));
...
function action_account_log ()
{
	...
	foreach($account_log as $k=>$v){		
			$rows['relmoney'] = str_replace('-', '', $v['money']);	
...
function action_act_account ()
{
	...
	$result = $GLOBALS['db']->getOne("select value from ecs_shop_config where code='charge'");
	//扣除提现手续费
	$money = $amount - $amount*$result/100;
	// var_dump($money);die;
	/* 变量初始化 */
	$surplus = array(
		'user_id' => $user_id, 'rec_id' => ! empty($_POST['rec_id']) ? intval($_POST['rec_id']) : 0, 'process_type' => isset($_POST['surplus_type']) ? intval($_POST['surplus_type']) : 0, 'payment_id' => isset($_POST['payment_id']) ? intval($_POST['payment_id']) : 0, 'user_note' => isset($_POST['user_note']) ? trim($_POST['user_note']) : '', 'amount' => $amount, 'money' => $money
	);
	...
	if($surplus['process_type'] == 1)
	{
		...
		$money = '-' . $money;
		$surplus['payment'] = '';
		$surplus['rec_id'] = insert_user_account($surplus, $amount, $money);
...
function action_cancel ()
{
	...
	if($id == 0 || $user_id == 0)
	{
		ecs_header("Location: usercenter.php?act=zijin\n");
		exit();
	}
