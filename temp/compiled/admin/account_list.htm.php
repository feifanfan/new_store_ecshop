...
<form method="post" action="account_log.php?act=list&user_id=<?php echo $_GET['user_id']; ?>" name="searchForm">
  ...
  <strong>收益</strong><?php echo $this->_var['user']['formated_user_money']; ?>
  <strong>重消</strong><?php echo $this->_var['user']['user_cash']; ?>
  <strong>企业币</strong><?php echo $this->_var['user']['user_point']; ?>
  </form>
</div>
...
?>
    <tr>
      ...
      <td align="right">
        <?php if ($this->_var['account']['user_cash'] > 0): ?>
          <span style="color:#0000FF">+<?php echo $this->_var['account']['user_cash']; ?></span>
        <?php elseif ($this->_var['account']['user_cash'] < 0): ?>
          <span style="color:#FF0000"><?php echo $this->_var['account']['user_cash']; ?></span>
        <?php else: ?>
          <?php echo $this->_var['account']['user_cash']; ?>
        <?php endif; ?>
      </td>
      <td align="right">
        <?php if ($this->_var['account']['user_point'] > 0): ?>
          <span style="color:#0000FF">+<?php echo $this->_var['account']['user_point']; ?></span>
        <?php elseif ($this->_var['account']['user_point'] < 0): ?>
          <span style="color:#FF0000"><?php echo $this->_var['account']['user_point']; ?></span>
        <?php else: ?>
          <?php echo $this->_var['account']['user_point']; ?>
        <?php endif; ?>
      </td>
     <!--  <td align="right">
